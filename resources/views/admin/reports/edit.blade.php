...
@section('content')
    {{-- Back link --}}
    <a href="{{ route('admin.reports.show', $report) }}" class="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700 mb-6">
    ...
    <x-card title="Edit Report">
        <form method="POST" action="{{ route('admin.reports.update', $report) }}" enctype="multipart/form-data">
            ...
            <div class="flex items-center gap-3 mt-8 pt-5 border-t border-gray-100">
                ...
                <a href="{{ route('admin.reports.show', $report) }}" class="text-sm text-gray-500 hover:text-gray-700 ml-auto">Cancel</a>
            </div>
        </form>
    </x-card>
@endsection
