...
@section('content')
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <p class="text-sm text-gray-500">View and manage all reports across the organization</p>
        </div>
        @can('create', App\Models\Report::class)
            <x-button variant="primary" :href="route('admin.reports.create')">
    ...
    {{-- Filters --}}
    <div x-data="{ showFilters: {{ request()->hasAny(['category', 'type', 'status', 'department_id', 'search', 'from_date', 'to_date']) ? 'true' : 'false' }} }" class="mb-6">
        ...
        <div x-show="showFilters" x-transition class="card px-6 py-4">
            <form method="GET" action="{{ route('admin.reports.index') }}">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <x-select name="type" label="Type" placeholder="All Types" :selected="request('type')"
                ...
                <div class="flex items-center gap-3 mt-4">
                    ...
                    <a href="{{ route('admin.reports.index') }}" class="text-sm text-gray-500 hover:text-gray-700">Clear</a>
                </div>
            </form>
        </div>
    </div>
    ...
    @if($reports->isEmpty())
        <x-card>
            <x-empty-state title="No reports found" description="No reports match your current filters." />
        </x-card>
    @else
        <x-data-table :headers="['author' => 'Author', 'title' => 'Title', 'department' => 'Department', 'category' => 'Category', 'type' => 'Type', 'status' => 'Status', 'date' => 'Date', 'actions' => '']">
            @foreach($reports as $report)
                <tr class="hover:bg-gray-50 cursor-pointer" onclick="window.location='{{ route('admin.reports.show', $report) }}'">
                    <td class="px-6 py-3 text-gray-900 font-medium whitespace-nowrap">{{ $report->user->full_name }}</td>
                    <td class="px-6 py-3">
                    ...
                    <td class="px-6 py-3 text-gray-500 whitespace-nowrap">{{ $report->department?->name ?? '-' }}</td>
                    ...
                    <td class="px-6 py-3" onclick="event.stopPropagation()">
                        <x-dropdown>
                            ...
                            <a href="{{ route('admin.reports.show', $report) }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">View</a>
                            @can('update', $report)
                                <a href="{{ route('admin.reports.edit', $report) }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">Edit</a>
                            @endcan
                            @can('delete', $report)
