...
@section('content')
    ...
    <x-card title="New Category">
        <form method="POST" action="{{ route('admin.video-categories.store') }}">
            ...
            <div class="space-y-5">
                <x-input name="name" label="Name" :value="old('name')" :error="$errors->first('name')" required placeholder="e.g. Training, Devotionals" />

                <div>
                    <label for="description" class="label">Description <span class="text-xs text-gray-400">(optional)</span></label>
                    <textarea name="description" id="description" rows="2" placeholder="Brief category description..."
                ...
                <x-input name="sort_order" label="Sort Order" type="number" :value="old('sort_order', 0)" :error="$errors->first('sort_order')" />
