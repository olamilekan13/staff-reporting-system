...
<div x-data="{
    ...
    async changePassword() {
        ...
        try {
            ...
            setTimeout(() => {
                window.location.href = '{{ route('login') }}?message=Password changed successfully. Please login.';
            }, 2000);
        } catch (e) {
            if (e.response?.status === 422) {
                const errors = e.response.data.errors;
                this.error = errors?.current_password?.[0] || errors?.new_password?.[0] || 'Failed to change password.';
    ...
    {{-- Success message --}}
    <div x-show="success" x-transition x-cloak class="mb-4 p-3 rounded-lg bg-green-50 border border-green-200">
    ...
    <div class="space-y-4">
        ...
        <a href="{{ url()->previous() }}" class="block w-full text-center text-sm text-gray-500 hover:text-gray-700">
            &larr; Cancel
        </a>
    </div>
</div>
