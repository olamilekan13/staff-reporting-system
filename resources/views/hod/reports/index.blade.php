...
@section('content')
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <p class="text-sm text-gray-500">Manage your reports and review department submissions</p>
        </div>
        <x-button variant="primary" :href="route('hod.reports.create')">
    ...
    {{-- Filters --}}
    <div x-data="{ showFilters: {{ request()->hasAny(['category', 'type', 'status', 'search', 'from_date', 'to_date']) ? 'true' : 'false' }} }" class="mb-6">
        ...
        <div x-show="showFilters" x-transition class="card px-6 py-4">
            <form method="GET" action="{{ route('hod.reports.index') }}">
                ...
                <div class="flex items-center gap-3 mt-4">
                    ...
                    <a href="{{ route('hod.reports.index') }}" class="text-sm text-gray-500 hover:text-gray-700">Clear</a>
                </div>
            </form>
        </div>
    </div>
    ...
    @if($reports->isEmpty())
        <x-card>
            <x-empty-state title="No reports found" description="Create a new report or adjust your filters.">
                <x-slot:action>
                    <x-button variant="primary" size="sm" :href="route('hod.reports.create')">Create Report</x-button>
                </x-slot:action>
            </x-empty-state>
        </x-card>
    @else
                ...
                <tr class="hover:bg-gray-50 cursor-pointer" onclick="window.location='{{ route('hod.reports.show', $report) }}'">
                    ...
                    <td class="px-6 py-3" onclick="event.stopPropagation()">
                        <x-dropdown>
                            ...
                            <a href="{{ route('hod.reports.show', $report) }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">View</a>
                            @can('update', $report)
                                <a href="{{ route('hod.reports.edit', $report) }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">Edit</a>
                            @endcan
                            @can('delete', $report)
                                <form method="POST" action="{{ route('hod.reports.destroy', $report) }}" onsubmit="return confirm('Are you sure you want to delete this report?')">
                                    @csrf
                                    @method('DELETE')
