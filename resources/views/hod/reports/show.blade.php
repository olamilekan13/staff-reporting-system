...
@section('content')
    {{-- Back link --}}
    <a href="{{ route('hod.reports.index') }}" class="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700 mb-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div>
                ...
                <div class="flex flex-wrap items-center gap-4 mt-3 text-sm text-gray-500">
                    <span>By {{ $report->user->full_name }}</span>
                    @if($report->department)
                        <span>&middot; {{ $report->department->name }}</span>
                    @endif
                    @if($report->submitted_at)
                        <span>&middot; Submitted {{ $report->submitted_at->format('M d, Y \a\t g:i A') }}</span>
                    @else
                        <span>&middot; Created {{ $report->created_at->format('M d, Y \a\t g:i A') }}</span>
                    @endif
                </div>
            </div>
            ...
            <div class="flex items-center gap-2 shrink-0">
                @can('update', $report)
                    <x-button variant="secondary" size="sm" :href="route('hod.reports.edit', $report)">
                ...
                <x-share-kingschat-button
                    ...
                    :url="route('hod.reports.show', $report)"
                    type="report"
                    variant="secondary"
                />
    ...
    @can('review', $report)
        @if($report->status === 'submitted')
            <x-card title="Review Report" class="mt-6">
                <form method="POST" action="{{ route('hod.reports.review', $report) }}">
                    @csrf

                    <div class="mb-4">
                        <label for="review_notes" class="label">Review Notes</label>
                        <textarea name="review_notes" id="review_notes" rows="3" placeholder="Add notes about your review decision..."
                            class="input @error('review_notes') border-red-300 @enderror">{{ old('review_notes') }}</textarea>
                        @error('review_notes')
    ...
    {{-- Comments section --}}
    <div class="mt-6" x-data="commentsSection({{ $report->id }})">
        <x-card>
